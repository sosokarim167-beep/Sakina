...
/* ============ مزامنة تقدّم قراءة القرآن (رقم الصفحة فقط — مبسّط بالكامل) ============ */
//
// تمت إزالة تتبّع السورة/الآية/موضع التمرير بالكامل كما طُلب صراحة.
// الوثيقة السحابية الآن تحمل حقل page فقط: { page: currentPage }
...
function syncQuranProgressToCloud(progress){
  ...
  return whenAuthReady().then(function(user){
    ...
    return setDoc(ref, {
      ...
      console.warn("Sakina: فشلت مزامنة رقم صفحة القرآن مع السحابة", err);
      return false;
    });
  });
}
...
function fetchQuranProgressFromCloud(){
  ...
  return whenAuthReady().then(function(user){
    ...
    return getDoc(ref).then(function(snap){
      ...
      console.warn("Sakina: فشل جلب رقم صفحة القرآن من السحابة", err);
      return null;
    });
  });
}
